...
工程主路线按学习阶段推进，不按自然周推进。每个阶段都有单独文档，包含阅读材料、概念清单、代码入口、练习任务和验收标准。

| Stage | 文档 | 概念重点 |
| --- | --- | --- |
| 00 | `notes/stage00_env.md` | 环境、版本、JIT、测试、benchmark |
| 00.25 | `notes/stage00_25_python_for_cpp.md` | C++ 到 Python/PyTorch 迁移、tensor、pytest |
| 00.5 | `notes/stage00_5_transformer_basics.md` | 深度学习张量、Q/K/V、attention 公式 |
| 01 | `notes/stage01_gpu_operator_model.md` | kernel、grid、thread、tensor、tile、memory |
| 02 | `notes/stage02_tilelang_dsl.md` | TileLang DSL、控制流、dtype、copy、reduction |
| 03 | `notes/stage03_memory_debug_perf.md` | global/shared/fragment、debug、latency、TFLOPS |
| 04 | `notes/stage04_gemm.md` | GEMM、Tensor Core、fragment accumulator、pipeline |
| 05 | `notes/stage05_autotune_operator_patterns.md` | autotuning、fusion、softmax、RMSNorm、访存估算 |
| 06 | `notes/stage06_attention_baseline.md` | attention、materialized matrix、online softmax |
| 07 | `notes/stage07_flash_attention.md` | FlashAttention forward、boundary guard、性能差距 |

学习时先读本文件建立词汇，再进入 Stage 00。由于之前主要有 C++ 背景、Python/PyTorch 较薄弱，可以先完成 Stage 00.25；如果对 `QK^T -> softmax -> P@V` 不熟，再完成 Stage 00.5，然后进入 TileLang kernel 主线。后续每进入一个新阶段，先读对应阶段文档，再回到本文件查概念。如果某个概念仍然不够清楚，继续读 `notes/concepts_deep_dive.md`，那里有更细的解释、代码对应关系、常见错误和自检问题。英文术语的常用中文表述见 `notes/glossary_zh_en.md`。

## 1. 什么是 AI 算子

AI 算子是深度学习模型中的一个计算单元，例如 matmul、softmax、layernorm、attention、conv、activation。框架层面看到的是 PyTorch 函数，底层真正执行的是 GPU kernel。
...
毕业目标是 FlashAttention forward，本质是把 attention 的 `QK^T -> softmax -> P@V` 融合成一个更省显存、更快的 kernel，其中 `P` 是 softmax 后的 attention probability。

## 2. TileLang 的位置

TileLang 是面向高性能张量程序的 DSL。使用它可以用 Python 写接近 CUDA/Triton 心智模型的代码，同时通过 TVM/TIR 等编译路径生成 GPU kernel。
...
和 PyTorch 相比，TileLang 需要显式思考线程、分块和内存层级；和 CUDA C++ 相比，TileLang 提供了更高层的 tensor tile、fragment、GEMM intrinsic 表达。
...
GEMM 是 AI 算子开发的核心练习。Linear、MLP、attention 的 `QK^T` 和 `P@V` 都可以看成 GEMM 或 batched GEMM。
...
这也是本工程 Stage 06 和 Stage 07 的主线。
...
2. 进入 Stage 00，确认环境、测试和 benchmark smoke。
3. Stage 01-02 重点掌握 TileLang DSL 和小 kernel 写法。
4. Stage 03-04 回看 5-7 节，理解 GEMM 的数据流和性能指标。
5. Stage 05 重点复习 8 和 10 节，把数值稳定、benchmark、fusion 串起来。
6. Stage 06-07 前重点复习 8-9 节，准备进入 FlashAttention。
7. 每写一个新算子，都用 `notes/operator_checklist.md` 做自检。
